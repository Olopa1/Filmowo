...
<!--Powyższe kilka linijek jest wygenerowane przez generator cookies-->
		...
		<div id="kontener">	
			...
			<div id="naj_oceniane_pf" class="wlasciwosci">
			<?php 
			$db = new SQLite3('sklep.db');
			if(!$db){ print $db->lastErrorMsg();}
			$zapytanie_filmy = "";
			if(isset($_GET['akcja']) and $_GET['akcja']=="usun_film"){
				$usun_film_zapytanie = "delete from Filmy where ID=:id_film";
				$przygotuj_usun_film = $db ->prepare($usun_film_zapytanie);
				$przygotuj_usun_film->bindValue(':id_film',$_GET['film_id']);
				$wykonaj = $przygotuj_usun_film -> execute();
			}
			if(isset($_POST['szukanie'])){
				$zapytanie_filmy = "select ID,Nazwa,Obrazek from Filmy where Nazwa like '%$_POST[szukanie]%'";
			}
			else{
				$zapytanie_filmy = "select ID,Nazwa,Obrazek from Filmy";
			}
			$wykonaj_filmy = $db->query($zapytanie_filmy);
			while($rzad = $wykonaj_filmy->fetchArray()){
				$pokaz_okladke="";
				if (count($rzad) > 0) {
					$pokaz_okladke=base64_encode($rzad[2]);
				}
			?>
				<div class="filmy" >
				...
				<a href="pokaz_film_szczegoly.php?id_film=<?php print $rzad[0] ?>"><div class="tytul_filmu"><?php print $rzad[1]; ?></div></a>
