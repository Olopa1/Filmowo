...
<!--Powyższe kilka linijek jest wygenerowane przez generator cookies-->
		...
		<div id="kontener">	
			...
			$wyswietl_kosz_kup = "select ID,ID_uzytkownik,ID_film from Posiadane_filmy where ID_uzytkownik = $id ";
			$wykonaj_kosz_kup = $db -> query($wyswietl_kosz_kup);
			while($rzad_kosz = $wykonaj_kosz_kup->fetchArray()){
				$zapytanie_filmy = "select ID,Nazwa,Obrazek from Filmy where ID = $rzad_kosz[2]";
			...
			?>
				...
				<a href="pokaz_film_szczegoly.php?id_film=<?php print $rzad[0] ?>&kupione=tak"><div class="tytul_filmu"><?php print $rzad[1]; ?></div></a>
