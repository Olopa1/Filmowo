...
<!--Powyższe kilka linijek jest wygenerowane przez generator cookies-->
		...
		<div id="kontener">	
			...
			<div> 
			<div id="wyswietl_film" class="wlasciwosci">
				<div class="tytul">Informacje o filmie <?php if($dostep < 3 ){ ?> <a href="konto.php?akcja_film=zmien_dane&wyswietl=dodaj_film&id_film=<?php print $_GET['id_film'] ?>">Zmień</a>  <a href="pokaz_film.php?akcja=usun_film&film_id=<?php  print $_GET['id_film'];?>"> <?php print 'Usuń</a>'; } ?></div>
				<?php 
			$db = new SQLite3('sklep.db');
			if(!$db){ print $db->lastErrorMsg();}
			$zapytanie_filmy = "";
			$zapytanie_filmy = "select ID,Nazwa,Rodzaj_filmu,Obrazek,Cena,Data_dodania,Scenarzysta,Dlugosc_filmu,Opis_filmu,Grupa_wiekowa from Filmy where ID='$_GET[id_film]'";
			$wykonaj_filmy = $db->query($zapytanie_filmy);
			$rzad = $wykonaj_filmy->fetchArray();
			$pokaz_okladke="";
			if (count($rzad) > 0) {
				$pokaz_okladke=base64_encode($rzad[3]);
			}
			?>
				<div class="filmy" >
				<div class="pozycja_zdj" ><img alt="brak obrazka" src="data:image/png;base64,<?= $pokaz_okladke ?>" class="okladka"></div>

				</div>
				<div id="informacje_film" style="float:left;" class="wlasciwosci">		
					Tytuł : <?php print $rzad[1] ?> 	
					<p>Reżyser :  <?php print $rzad[6] ?>   </p>
					<p>Długość filmu : <?php print $rzad[7] ?>  </p>
					<p>Grupa wiekowa : <?php print $rzad[9] ?>  </p>
				</div>
				<div class="wlasciwosci" id="opis_filmu">
				Opis filmu : <?php print $rzad[8]; ?>
				</div>
				<div class="wlsciwosci" id="cena_filmu">Cena : <?php print $rzad[4]; ?> cebulionów  <?php if(!isset($_GET['kupione'])){ ?><div id="dodaj_do_koszyka" ><a href="koszy.php?id_film= <?php print $rzad[0] ?>"><img id="koszyk" src="./grafiki/koszyk_bez_klik.png" onmouseover="this.src='./grafiki/koszyk_klik.png'" onmouseout="this.src='./grafiki/koszyk_bez_klik.png'"></a> </div><?php } ?></div>
				</div>			
			</div>
